...
{
	usmart_dev.runtime+=HAL_GetTick();
...
//如果非ALIENTEK用户,则USART_RX_STA和usmart_buf[]需要用户自己实现
void usmart_scan(void)
{
	uint8_t sta,len;
	
	len = strlen((const char*)usmart_buf);
	if(len<4)
	{
		return;
	}
	usmart_buf[len]='\0';	//在末尾加入结束符. 
	sta=usmart_dev.cmd_rec(usmart_buf);//得到函数各个信息
	if(sta==0)usmart_dev.exe();	//执行函数 
	else 
	{  
		len=usmart_sys_cmd_exe(usmart_buf);
